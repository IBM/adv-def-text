...
def filter_emb(vocab_file, emb_file, output_path):
    ...
    for word in vocab:
        if word in emb_map:
            ...
            raise OSError
    json.dump(emb_matrix, open(output_path, 'w'))
...
def snli_extract_(lines):
    ...
    for ind, line in enumerate(lines):
        if ind == 0:
            continue
        comps = line.strip().split('\t')
        sentences.append(comps[5].lower() + ' [SEP] ' + comps[6].lower())
...
def split_snli_input(input_file, output_path):
    ...
    for ind, sent in enumerate(sents):
        comps = sent.split(' [ SEP ] ')
        if len(comps) < 2:
            print(str(ind+1))
            continue
        text = comps[0].strip()+' .' if (not comps[0].strip().endswith('.')) else comps[0]
        hypo = comps[1].strip()+' .' if (not comps[1].strip().endswith('.')) else comps[1]
        texts.append(text)
        hypos.append(hypo)
        new_sents.append(text+' <sep> '+hypo)

    utils.write_lines(texts, output_path+'.prem.in')
    utils.write_lines(hypos, output_path+'.hypo.in')
    utils.write_lines(new_sents, output_path + '.in')
...
if __name__ == '__main__':
    ...
    split_snli_input('/dccstor/ddig/ying/data/snli/train.backup.in', '/dccstor/ddig/ying/data/snli/train')
    split_snli_input('/dccstor/ddig/ying/data/snli/dev.backup.in', '/dccstor/ddig/ying/data/snli/dev')
    split_snli_input('/dccstor/ddig/ying/data/snli/test.backup.in', '/dccstor/ddig/ying/data/snli/test')
